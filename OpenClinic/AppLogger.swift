...
/// Centralized logger factory for the entire OpenClinic app.
/// Every subsystem gets its own `os.Logger` category so you can filter
/// in Console.app or Xcode with:  `subsystem:com.openclinic` + `category:AI`
enum AppLogger {
    private nonisolated static let subsystem = Bundle.main.bundleIdentifier ?? "com.openclinic"
    ...
    /// SMART on FHIR discovery, OAuth, and token exchange
    nonisolated static let smart     = Logger(subsystem: subsystem, category: "SMART")
}
